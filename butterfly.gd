...
func flap_down():
	...
	flap_tween.tween_property(lwing,"rotation",Vector3(lwing.rotation.x,lwing.rotation.y,deg_to_rad(220)),0.5 / flap_speed)
	flap_tween.set_parallel()
	flap_tween.tween_property(rwing,"rotation",Vector3(rwing.rotation.x,rwing.rotation.y,deg_to_rad(-40)),0.5 / flap_speed)
	flap_tween.tween_callback(self.flap_up)

func flap_up():
	if flap_tween: flap_tween.cancel_free()
	flap_tween = get_tree().create_tween()
	flap_tween.tween_property(lwing,"rotation",Vector3(lwing.rotation.x,lwing.rotation.y,deg_to_rad(140)),0.5 / flap_speed)
	flap_tween.set_parallel()
	flap_tween.tween_property(rwing,"rotation",Vector3(rwing.rotation.x,rwing.rotation.y,deg_to_rad(40)),0.5 / flap_speed)
	flapping = false
